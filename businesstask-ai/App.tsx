...
import { LayoutDashboard, MessageSquare, Layers, ChevronRight } from 'lucide-react';
import { ChatInterface } from './components/ChatInterface';
import { TaskBoard } from './components/TaskBoard';
import { OnboardingChat } from './components/OnboardingChat';
import { BusinessOverviewModal } from './components/BusinessOverviewModal';
import { BusinessProfile, ChatMessage, Task, BusinessStage, StrategicAnalysis } from './types';
import { generateTasksFromContext, generateStrategicAnalysis } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  // Initial Task Generation Handler
  const handleGenerateTasks = async () => {
    if (!profile) return;
    setIsGeneratingTasks(true);
    
    // Add a system message to chat to indicate analysis
    ...
    if (newTasks.length > 0) {
      setTasks(prev => {
        // MERGE STRATEGY: MAX 7 Active Tasks (Todo + In-Progress)
        // 1. Keep all Completed tasks (for history).
        // 2. Keep all In-Progress tasks (User is working on them).
        // 3. Add all New Tasks (They are the new priority).
        // 4. Fill remaining slots with Old Todos (High Priority first).
        ...
        // Remove duplicates from newTasks if they match existing titles in prev
        ...
        if (slotsRemaining > 0) {
          // Sort old todos by priority (High -> Medium -> Low)
      ...
      // Add a follow up message indicating tasks are ready
      setTimeout(() => {
        setChatHistory(prev => [...prev, {
      ...
      // Auto-switch to dashboard if tasks are generated
  ...
  const handleOnboardingComplete = async (newProfile: BusinessProfile, history: ChatMessage[]) => {
    ...
    // Immediately run strategic analysis
    const strategicData = await generateStrategicAnalysis(newProfile, history);
    setAnalysis(strategicData);
    setShowOverview(true); // Auto-show the overview once ready
  };

  // Trigger initial task generation when profile is first set via onboarding
  useEffect(() => {
